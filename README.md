...
- **Half-Duplex Selection** (Single Wire, `HAL_HalfDuplex_Init()` в MX-коде) —
  ...
  с одним пином вместо раздельных RX/TX). Библиотека при этом всё равно
  только принимает — обратная телеметрия (i-BUS SENSOR — исключение, см.
  ниже) не отправляется, см. "Честные ограничения".
