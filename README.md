...
управления по FlySky i-BUS (пример приёмника — FS-iA6B), Futaba S.BUS
(пример — приёмники RadioMaster с S.BUS-выходом) или CRSF (протокол TBS
Crossfire / ExpressLRS), и ответ телеметрией на опрос по отдельной шине
датчиков i-BUS. Работает поверх стандартного HAL UART/DMA, без bit-bang и
без блокирующего опроса в основном цикле.
...
USART global interrupt включён. CRSF используется только на приём (нога RX
МК подключается к выходу приёмника, TX МК можно не подключать — библиотека
не отправляет телеметрию обратно в приёмник по CRSF, см. "Честные
ограничения").
