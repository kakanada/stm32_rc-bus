...
таблицы выше, сам Mode не проверяется — но если в `Instance->CR3` обнаружен
бит `HDSEL` (Half-Duplex включён), `RCBUS_Init()`/`RCBUS_UART_RxEventCallback()`
перед каждым (пере)запуском приёма сами вызывают
`HAL_HalfDuplex_EnableReceiver()`, фиксируя направление "только приём"
(`TE=0`/`RE=1`) — иначе собственный передатчик МК держит общий провод в
состоянии "mark" и забивает сигнал приёмника. В Half-Duplex режиме провод
приёмника подключается к ноге **TX** МК, а не RX (особенность периферии
STM32 — RX-пин в этом режиме не используется).
